...
class Upload{
    ...
    public function upload($arquivo, $pasta){
        ...
        $dataMes = date("m");
        $dataAno = date("Y");
        $caminho = DIR_PATH.'/uploads/'.$pasta.'/'. $dataAno . '/' . $dataMes . '/'  . $dataDia . '/';
        ...
        if ($_FILES[$arquivo]['name'] !== '' && $_FILES[$arquivo]['size'] > 0) {
            ...
            if (array_search($extensao, $_UP['extensoes']) === false) {
                return "Arquivo ".$_FILES[$arquivo]['name']." está fora do padrão! Por favor, envie o documento na extensão solicitada: PNG ou JPG";
                exit();
            }
            ...
            if (move_uploaded_file($_FILES[$arquivo]['tmp_name'], $_UP['pasta'] . $nome_final)) {
                ...
                $docLiq = 'uploads/'.$pasta.'/'. $dataAno . '/' . $dataMes . '/'  . $dataDia . '/' . $nome_final;
                return $docLiq;
            } else {
